...
class TestParametresDesTraductions:
    ...
    # Certaines entrées ne sont pas des phrases : la table des
    # profils de collection porte des tuples, que `t_msg` lit tels
    # quels. Les traiter comme du texte n'apprendrait rien.
    NON_TEXTUELLES = {"profils_biohot", "termes_personne"}
    ...
    def test_aucun_texte_vide(self):
        # Un fragment de prompt VIDE est un choix, non un oubli : la
        # partie française est validée avant d'être traduite, et une
        # clé ABSENTE ferait retomber sur l'anglais — mélanger deux
        # langues dans un prompt tire le modèle vers la mauvaise.
        en_attente = {"prompt_biohot_apport"}
        for langue, bloc in i18n.CATALOGUE.items():
            for famille in ("tags", "boutons", "taches", "reglages",
                ...
                if cle in self.NON_TEXTUELLES:
                    ...
                    if cle in en_attente:
                        continue
                    assert str(texte).strip(), \
                        f"{langue}/{famille}/{cle}"
