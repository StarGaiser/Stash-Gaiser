...
class TestMarqueurToujoursConsomme:
    """Un studio accepté dont aucune source n'a rien à dire sortait
    par un retour anticipé, AVANT l'effacement du marqueur.

    La fiche était donc reprise à chaque passage : les sources
    réinterrogées, le modèle rappelé, et rien ne changeait jamais. La
    tâche annonçait « 1 studio appliqué » sans effet visible, et rien
    ne permettait de deviner qu'elle recommencerait indéfiniment.

    Le coût n'est pas seulement l'inutilité : ce sont des appels
    payants, répétés à chaque exécution, sur des fiches qui ne
    changeront pas."""

    def _monde(self, **cf):
        st = FauxStash(studios=[studio(1, "Studio Sans Source",
                                       custom_fields=cf)])
        ctx = faux_contexte({}, st)
        ctx.args = {}
        return st, ctx

    def test_rien_a_appliquer_consomme_quand_meme(self,
                                                  monkeypatch):
        monkeypatch.setattr(studios, "collecter_studio",
                            lambda *a, **k: {})
        monkeypatch.setattr(studios, "stats_studio",
                            lambda *a, **k: {})
        st, ctx = self._monde(enrich_accept="1")
        studios.apply_accepted_studios(ctx)
        cf = st.studios["1"].get("custom_fields") or {}
        assert not cf.get("enrich_accept")

    def test_une_fiche_non_acceptee_n_est_pas_touchee(self,
                                                      monkeypatch):
        """Le marqueur absent veut dire « pas de décision » : y
        toucher effacerait ce que l'utilisateur n'a pas encore
        arbitré."""
        monkeypatch.setattr(studios, "collecter_studio",
                            lambda *a, **k: {})
        monkeypatch.setattr(studios, "stats_studio",
                            lambda *a, **k: {})
        st, ctx = self._monde()
        avant = dict(st.studios["1"])
        studios.apply_accepted_studios(ctx)
        assert st.studios["1"].get("name") == avant.get("name")

    def test_l_enrichissement_ordinaire_ne_consomme_pas(self,
                                                        monkeypatch):
        """Hors application d'une acceptation, le marqueur n'a rien à
        faire là : l'effacer viderait une décision en attente."""
        monkeypatch.setattr(studios, "collecter_studio",
                            lambda *a, **k: {})
        monkeypatch.setattr(studios, "stats_studio",
                            lambda *a, **k: {})
