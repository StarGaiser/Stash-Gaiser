...
class FauxStash:
    ...
    def __init__(self, performers=None, scenes=None, studios=None,
                 tags=None, groups=None, reglages_plugin=None):
        ...
        # La table des réglages du plugin, que `configurePlugin`
        # remplace en entier — c'est le piège que ce faux doit
        # reproduire, non adoucir.
        self.reglages_plugin = dict(reglages_plugin or {})
    ...
    def find_scene(self, ident, fragment=None):
        self._note("find_scene")
        sc = self.scenes.get(str(ident))
        return copy.deepcopy(sc) if sc else None
    ...
    def find_studio(self, ident, create=False, fragment=None):
        self._note("find_studio")
        st = self.studios.get(str(ident))
        return copy.deepcopy(st) if st else None

    def find_studios(self, f=None, filter=None, fragment=None):
        self._note("find_studios")
        return [copy.deepcopy(x) for x in self.studios.values()]
    ...
    def call_GQL(self, requete, variables=None, callback=None):
        ...
        for nom, action in (
            ...
            # L'ordre compte : « configurePlugin » doit être
            # reconnu AVANT « configuration », sans quoi une mutation
            # serait traitée comme une lecture et n'écrirait rien.
            ("configurePlugin", self._configure),
            ("configuration { plugins", self._lire_config),
    ...
    def _lire_config(self, v):
        self._note("configuration")
        """La table des réglages, telle que le serveur la rendrait.

        `configurePlugin` REMPLACE cette table : tout code qui écrit
        doit la relire d'abord, et c'est précisément ce qu'un test
        doit pouvoir vérifier."""
        return {"configuration": {"plugins": {
            "gaizer": dict(self.reglages_plugin)}}}

    def _configure(self, v):
        # Le nom de la variable GraphQL varie selon l'appelant — « i »
        # ou « input » : ce qui compte est la VALEUR, non son
        # étiquette.
        table = v.get("input", v.get("i", {}))
        # Remplacement, pas fusion : c'est le comportement réel, et
        # le simuler autrement masquerait le piège qu'il tend.
        self.reglages_plugin = dict(table)
        return {"configurePlugin": table}
