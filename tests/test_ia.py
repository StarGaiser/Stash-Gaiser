...
class TestApercuSansEcrire:
    """Générer un texte pour voir ne doit RIEN écrire sur le champ
    visible : sans cela, l'aperçu serait un fait accompli.

    Le texte est déposé dans un champ à part, où le panneau de la
    fiche le lit. L'utilisateur accepte ou refuse depuis là."""

    def _monde(self, **reglages):
        base = {"aiBiohot": "mistral:m", "llmApiKey": "essai",
                "language": "fr"}
        base.update(reglages)
    ...
    def test_le_champ_visible_n_est_pas_touche(self, monkeypatch):
        monkeypatch.setattr(ia, "_appel_llm",
                            lambda *a, **k: "Texte généré.")
        st, ctx = self._monde()
        ia.generer_apercu(ctx)
        assert st.performers["1"]["details"] == "Un texte."

    def test_le_texte_est_depose_a_part(self, monkeypatch):
        monkeypatch.setattr(ia, "_appel_llm",
                            lambda *a, **k: "Texte généré.")
        st, ctx = self._monde()
        ia.generer_apercu(ctx)
        cf = st.performers["1"].get("custom_fields") or {}
        assert cf.get("enrich_apercu") == "Texte généré."

    def test_sans_modele_rien_ne_se_passe(self):
        st, ctx = self._monde(aiBiohot="", aiDefault="")
        ia.generer_apercu(ctx)
        cf = st.performers["1"].get("custom_fields") or {}
        assert not cf.get("enrich_apercu")
    ...
    def test_une_fiche_absente_ne_leve_pas(self):
        st, ctx = self._monde()
        ctx.args = {"performer_id": "999"}
        ia.generer_apercu(ctx)

    def test_un_modele_muet_ne_depose_rien(self, monkeypatch):
        monkeypatch.setattr(ia, "_appel_llm", lambda *a, **k: None)
        st, ctx = self._monde()
        ia.generer_apercu(ctx)
        cf = st.performers["1"].get("custom_fields") or {}
        assert not cf.get("enrich_apercu")

    def test_un_studio_emploie_son_propre_texte(self, monkeypatch):
        """Une biographie n'est pas une présentation de studio : le
        même prompt pour les deux produirait deux textes également
        inadaptés."""
        vus = []
        monkeypatch.setattr(ia, "synth_bio",
                            lambda *a, **k: vus.append("studio")
                            or "Présentation.")
        st = FauxStash(studios=[studio(9, "Hard Kinks",
                                       details="Un studio.")])
        ctx = faux_contexte({"aiBio": "mistral:m",
                             "llmApiKey": "essai"}, st)
        ctx.args = {"studio_id": "9"}
        ia.generer_apercu(ctx)
        assert vus == ["studio"]

    def test_une_scene_emploie_le_synopsis(self, monkeypatch):
        vus = []
        monkeypatch.setattr(ia, "synth_synopsis",
                            lambda *a, **k: vus.append("scene")
                            or "Synopsis.")
        st = FauxStash(scenes=[scene(10, "Une scène",
                                     details="Une scène.")])
        ctx = faux_contexte({"aiSynopsis": "mistral:m",
                             "llmApiKey": "essai"}, st)
        ctx.args = {"scene_id": "10"}
        ia.generer_apercu(ctx)
        assert vus == ["scene"]


class TestPlafondDeReponse:
    """La réponse du modèle était coupée à 480 caractères, quel que
    soit l'usage. Le plafond protège d'un fournisseur qui déverserait
    des kilo-octets, mais 480 est la taille d'UNE biographie courte :
    une présentation de quatre phrases denses s'y termine au milieu
    d'un mot.

    Un texte tronqué est pire qu'un texte plat — il se voit, il gêne,
    et il donne l'impression d'une panne."""

    def test_le_plafond_suit_le_budget_demande(self, monkeypatch):
        """Demander quatre cents jetons puis couper à quatre cent
        quatre-vingts signes est contradictoire : le second plafond
        annule le premier."""
        from pathlib import Path as _P
        code = (_P(ia.__file__)).read_text(encoding="utf-8")
        assert "[:480]" not in code, \
            "un plafond fixe annule les budgets par usage"

    def test_un_plafond_subsiste(self):
        """Le retirer laisserait un fournisseur défaillant remplir la
        fiche de kilo-octets."""
        from pathlib import Path as _P
        code = (_P(ia.__file__)).read_text(encoding="utf-8")
        i = code.find("lire_reponse(conf, r.read())")
        bloc = code[i:i + 200]
        assert "[:" in bloc, bloc[:120]

    def test_le_plafond_laisse_passer_une_presentation(self,
                                                       monkeypatch):
        """Quatre phrases denses font environ six cents signes."""
        long = "x" * 900
        monkeypatch.setattr(
            ia.llm, "lire_reponse", lambda *a, **k: long)
        ...
        monkeypatch.setattr(ia.noyau, "url_sure", lambda u: True)
        texte = ia._appel_llm_une_fois(
            "mistral", "m", "cle", "prompt", 0.7,
            {"url": "https://x.test/v1", "format": "openai"},
            {}, 400)
        assert len(texte) >= 700, len(texte)


class TestNomsPropresVerifies:
    """Le prompt dit explicitement que les noms propres se recopient
    et ne s'estiment pas. Le modèle a produit « Titan Men » sur une
    fiche dont les studios réels sont tout autres.

    L'enseignement est net : une consigne ne suffit pas contre ce
    penchant. Un nom de studio plausible est exactement ce qu'un
    modèle produit le mieux, et personne ne vérifie un nom qui sonne
    juste.

    Le contrôle doit donc être MÉCANIQUE, après génération : les noms
    de studio cités sont comparés à ceux fournis, et le texte est
    refusé s'il en invente un. Refuser coûte un appel ; laisser passer
    coûte une fiche fausse que personne ne corrigera."""

    def test_un_studio_absent_des_donnees_fait_refuser(self):
        texte = "Quatre scènes chez Titan Men, toutes excellentes."
        assert not ia.noms_verifies(texte, {"Drill My Hole",
                                            "Jizz Orgy"})

    def test_un_studio_present_passe(self):
        texte = "Quatre scènes chez Drill My Hole."
        assert ia.noms_verifies(texte, {"Drill My Hole", "Jizz Orgy"})

    def test_un_texte_sans_studio_passe(self):
        """Ne nommer aucun studio est le comportement attendu quand
        les données n'en donnent pas."""
        assert ia.noms_verifies("Corps épais, 20 cm.", set())

    def test_la_casse_et_les_espaces_sont_ignores(self):
        assert ia.noms_verifies("chez DRILL MY HOLE",
                                {"Drill My Hole"})

    def test_les_mots_courants_ne_sont_pas_pris_pour_des_studios(self):
        """« Il » ou « Corps » commencent par une majuscule en début
        de phrase sans être des noms de studio."""
        texte = ("Il baise sans préliminaires. Corps épais. "
                 "Quatre scènes ici.")
        assert ia.noms_verifies(texte, set())

    def test_les_prenoms_de_partenaires_sont_admis(self):
        """Un nom de personne fourni dans les données n'est pas un
        studio inventé."""
        assert ia.noms_verifies("souvent avec Dean Young",
                                {"Dean Young"})

    def test_valeurs_absurdes(self):
        for texte in ("", None):
            assert ia.noms_verifies(texte, {"X"}) in (True, False)


class TestPourquoiVerifierLesNoms:
    """Une question a été posée : « Titan Men » est-il vraiment faux,
    au-delà de la collection ?

    Vérification faite, aucune source ne rattache cet acteur à ce
    studio. Mais la réponse ne change RIEN au contrôle, et c'est le
    point important.

    Le texte affirmait « quatre scènes ici, toutes chez Titan Men » —
    « ici » désignant la collection. Or les quatre scènes de cette
    collection sont chez d'autres studios. L'affirmation était fausse
    QUELLE QUE SOIT la filmographie réelle de l'acteur.

    Un texte ne peut affirmer que ce que les données montrent. Le
    modèle sait des choses que le plugin ignore, et ce savoir est
    précisément ce qu'il ne faut pas laisser entrer : rien ne permet
    de le vérifier au moment où il est écrit."""

    def test_le_controle_porte_sur_les_donnees_fournies(self):
        """Un studio réel de l'acteur, mais absent des données de
        cette fiche, reste refusé : le texte parle de CETTE
        collection."""
        assert not ia.noms_verifies(
            "Quatre scènes ici, toutes chez Men Com.",
            {"Drill My Hole", "Jizz Orgy"})

    def test_un_studio_des_donnees_passe(self):
        assert ia.noms_verifies(
            "Quatre scènes chez Drill My Hole.",
            {"Drill My Hole", "Jizz Orgy"})

    def test_le_savoir_du_modele_n_est_pas_une_source(self):
        """Ce que le modèle croit savoir n'est vérifiable ni par le
        plugin, ni par le lecteur au moment où il lit."""
        assert not ia.noms_verifies(
            "Ancien Marine, il a servi en Afghanistan.", set())
