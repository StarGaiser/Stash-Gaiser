...
class TestRepliParNomDeFichier:
    """Quand aucune empreinte ne reconnaît le fichier, le plugin tente
    de lire le nom du fichier. C'est le seul chemin du plugin qui
    DEVINE, et il doit être le plus prudent.

    Une identification par empreinte est sûre : le fichier est reconnu,
    les données lui appartiennent. Un nom de fichier ne prouve rien —
    il peut contenir n'importe quoi, y compris le nom d'un studio qui
    n'a pas produit la scène."""

    def _monde(self, basename, mode="auto"):
        st = FauxStash(
            scenes=[scene(10, "", files=[{"basename": basename}])],
            studios=[studio(1, "Masqulin"), studio(2, "Falcon Studios")],
            performers=[performer(1, "Archie Fox"),
                        performer(2, "Dean Young")])
        ctx = faux_contexte({"applyMode": mode}, st)
        ctx.args = {}
        return st, ctx

    def test_un_studio_reconnu_est_applique(self):
        st, ctx = self._monde("Masqulin - Une scene.mp4")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        assert (st.scenes["10"].get("studio") or {}).get("id") == "1"

    def test_un_interprete_reconnu_est_relie(self):
        st, ctx = self._monde("Masqulin - Archie Fox.mp4")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        ids = {q["id"] for q in st.scenes["10"].get("performers") or []}
        assert "1" in ids

    def test_un_nom_trop_court_ne_declenche_rien(self):
        """Un fragment de cinq lettres se retrouve par hasard dans
        quantité de noms de fichiers : exiger une longueur minimale
        évite d'attribuer une scène au premier studio venu."""
        st = FauxStash(
            scenes=[scene(10, "", files=[{"basename": "men_2023.mp4"}])],
            studios=[studio(1, "Men")])
        ctx = faux_contexte({"applyMode": "auto"}, st)
        ctx.args = {}
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        assert not st.scenes["10"].get("studio")

    def test_rien_de_reconnu_marque_la_scene(self):
        """Une scène que rien n'identifie doit devenir FILTRABLE, pas
        se perdre dans un journal que personne ne relit."""
        st, ctx = self._monde("video_2023_final.mp4")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        noms = {t["name"] for t in st.scenes["10"].get("tags") or []}
        assert any("identif" in n for n in noms)

    def test_le_titre_est_tire_du_nom_de_fichier(self):
        st, ctx = self._monde("Masqulin - Une belle scene.mp4")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        titre = st.scenes["10"].get("title") or ""
        assert "Une belle scene" in titre
        assert ".mp4" not in titre

    def test_un_titre_existant_n_est_pas_ecrase(self):
        st = FauxStash(
            scenes=[scene(10, "Mon titre à moi",
                          files=[{"basename": "Masqulin - Autre.mp4"}])],
            studios=[studio(1, "Masqulin")])
        ctx = faux_contexte({"applyMode": "auto"}, st)
        ctx.args = {}
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        assert st.scenes["10"]["title"] == "Mon titre à moi"

    def test_un_studio_existant_n_est_pas_ecrase(self):
        st = FauxStash(
            scenes=[scene(10, "", studio={"id": "2"},
                          files=[{"basename": "Masqulin - X.mp4"}])],
            studios=[studio(1, "Masqulin"), studio(2, "Falcon Studios")])
        ctx = faux_contexte({"applyMode": "auto"}, st)
        ctx.args = {}
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        assert st.scenes["10"]["studio"]["id"] == "2"

    def test_sans_fichier_rien_ne_se_passe(self):
    ...
    def test_en_mode_manuel_rien_n_est_applique(self):
        """Le repli devine : en mode manuel, il ne doit pas écrire
        plus que la collecte par empreinte."""
        st, ctx = self._monde("Masqulin - Archie Fox.mp4",
                              mode="manual")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        assert not st.scenes["10"].get("studio")

    def test_la_provenance_dit_que_c_est_une_devinette(self):
        """L'utilisateur doit pouvoir distinguer une valeur lue dans un
        nom de fichier d'une valeur confirmée par une source."""
        st, ctx = self._monde("Masqulin - Archie Fox.mp4")
        scenes._repli_nom_fichier(ctx, st.scenes["10"], False)
        cf = st.scenes["10"].get("custom_fields") or {}
        trace = (str(cf.get("enrich_sources") or "")
                 + str(cf.get("enrich_rapport") or "")).lower()
        # Le libellé suit la langue du contexte : c'est le SENS qui
        # doit être présent, et l'aveu de moindre confiance avec lui.
        assert any(m in trace for m in ("fichier", "file name"))
        assert any(m in trace for m in
                   ("empreinte", "fingerprint", "confiance",
                    "confidence"))
