...
class TestVocabulaire:
    ...
    def test_aucun_sigle_non_developpe(self):
        ...
        connus = {"API", "URL", "JSON", "YAML", "HTML", "CSS", "SQL",
                  ...
                  "TESTS", "READY", "BR", "VEUT",
                  # Principes de conception, chacun
                  # expliqué dans sa section.
        ...
        for f in _documents():
            for sigle in re.findall(r"\b([A-Z]{2,6})\b", _texte(f)):
                ...
                if re.search(rf"{sigle}\s*[—(-]", _texte(f)):
                    continue
                # Cité entre accents graves : c'est du texte brut —
                # un identifiant, un exemple de nom de fichier —, non
                # du vocabulaire que le lecteur doit connaître.
                import re as _re
                cite = any(sigle in bloc for bloc in
                           _re.findall(r"`([^`]+)`", _texte(f)))
                if cite:
                    continue
