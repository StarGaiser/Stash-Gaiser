...
class TestNomsCreables:
    """Une source peut renvoyer n'importe quoi, et le plugin CRÉE des
    fiches à partir de ce qu'elle dit. Un identifiant interne, un
    fragment de nom de fichier, un code d'encodage : chacun devient
    une fiche d'interprète que personne ne remarquera avant longtemps.

    Une fiche créée à tort est pire qu'une scène incomplète : elle
    pollue le catalogue, fausse les rapprochements ultérieurs, et
    prive la scène de son vrai interprète.

    Le contrôle porte sur la FORME. Il ne peut pas savoir si « Archie
    Fox » existe, mais il sait que « Do 40092 » n'est pas un nom."""

    @pytest.mark.parametrize("brut", [
        "Do 40092", "Tony 40365", "Perf 123456", "X 9999",
        "H264", "NOTAG", "1080p", "AAC 2 0",
        "12345", "id 8821", "user_4471",
    ])
    def test_un_identifiant_n_est_pas_un_nom(self, brut):
        assert not entites.nom_creable(brut), brut

    @pytest.mark.parametrize("brut", [
        "Archie Fox", "Dean Young", "José Domínguez",
        "Marie-Claude Durand", "Bo Sinn", "D'Angelo Jackson",
        "Mikel Bosco", "Hugo Vergari",
    ])
    def test_un_vrai_nom_passe(self, brut):
        assert entites.nom_creable(brut), brut

    def test_un_nom_avec_un_chiffre_reste_refuse(self):
        """« Tony 40365 » : un nombre à quatre chiffres ou plus dans
        un nom d'interprète est un identifiant, jamais un patronyme."""
        assert not entites.nom_creable("Tony 40365")

    def test_un_prenom_suivi_d_un_chiffre_est_refuse(self):
        """« Rocco 2 » pourrait distinguer deux homonymes, mais vient
        bien plus souvent d'un découpage raté. Une fiche créée à tort
        coûte plus qu'une fiche manquante : l'utilisateur peut créer
        l'homonyme lui-même, il ne verra pas passer le fantôme."""
        assert not entites.nom_creable("Rocco 2")

    def test_un_mot_seul_est_refuse(self):
        """Un mot isolé désigne trop de monde, et vient souvent d'un
        découpage raté."""
        for brut in ("Do", "Archie", "MEN", "Gay"):
            assert not entites.nom_creable(brut), brut

    def test_les_valeurs_vides(self):
        for brut in ("", None, "  ", "x"):
            assert not entites.nom_creable(brut), brut

    def test_un_nom_trop_long_est_refuse(self):
        """Une phrase entière vient d'une hallucination ou d'un champ
        mal lu."""
        assert not entites.nom_creable("Le meilleur des scènes de "
                                       "ce studio en haute définition")

    def test_la_creation_refuse_un_nom_invalide(self):
        """Le contrôle doit être DANS la création, non chez l'appelant :
        toute source qui crée doit en bénéficier."""
        st = FauxStash()
        ctx = faux_contexte({"createMissing": True}, st)
        avant = len(st.performers)
        entites._creer_performer_minimal(ctx, "Do 40092", {})
        assert len(st.performers) == avant

    def test_la_creation_accepte_un_nom_valide(self):
        st = FauxStash()
        ctx = faux_contexte({"createMissing": True}, st)
        entites._creer_performer_minimal(ctx, "Archie Fox", {})
        assert any(p["name"] == "Archie Fox"
                   for p in st.performers.values())
