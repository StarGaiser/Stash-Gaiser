...
def deduire_roles(ctx):
    """Cherche dans la documentation déjà collectée une mention
    explicite du rôle, pour les fiches qui n'en ont pas.

    Aucune source ne renseigne ce champ ; la seule piste est qu'un
    texte le DISE. Le modèle de langage sert donc à lire, pas à
    supposer : il doit citer le passage, et cette citation est
    vérifiée présente dans les textes fournis avant d'être retenue.
    Une fiche sans mention reste vide, ce qui est le résultat attendu
    dans la plupart des cas.

    Ce qui est trouvé est marqué « suggéré » sur la fiche : le
    panneau le signale, et modifier la valeur vaut confirmation."""
    perfs = ctx.stash.find_performers()
    cibles = [p for p in perfs
              if not (p.get("custom_fields") or {}).get("enrich_position")
              and not (p.get("custom_fields") or {}).get("enrich_pouvoir")]
    limite = ctx.batch()
    log.info(f"{len(cibles)} fiche(s) sans rôle — lot de {limite}")

    trouves = vides = 0
    motifs = {}
    for i, p in enumerate(cibles[:limite], 1):
        cf = p.get("custom_fields") or {}
        # Ce qui est déjà sur la fiche, rien de plus : la déduction ne
        # va pas chercher ailleurs, elle relit.
        # La présentation « hot » est EXCLUE : elle a été rédigée par
        # le modèle à partir de données factuelles, et rien ne garantit
        # que ses tournures reposent sur une mention réelle. Y lire un
        # rôle reviendrait à confirmer une invention par elle-même.
        # Seuls comptent les textes venus des sources.
        morceaux = [p.get("details") or ""]
        try:
            reco = json.loads(cf.get("reco_data") or "{}")
            if reco.get("tags"):
                morceaux.append("Étiquettes de ses scènes : "
                                + ", ".join(reco["tags"][:25]))
        except (ValueError, TypeError):
            pass
        morceaux += [t["name"] for t in (p.get("tags") or [])]
        for a in (p.get("alias_list") or []):
            morceaux.append(a)
        evidences = "\n".join(x for x in morceaux if str(x).strip())
        if len(evidences) < 40:
            vides += 1
            continue

        lu, motif = deduire_role(ctx, p, evidences)
        if not lu:
            vides += 1
            motifs[motif] = motifs.get(motif, 0) + 1
            continue
        maj = {"enrich_" + k: v for k, v in lu.items()}
        maj["enrich_role_origine"] = "ia"
        maj["enrich_role_motif"] = str(motif)[:200]
        try:
            ctx.stash.update_performer({
                "id": p["id"], "custom_fields": {"partial": maj}})
            trouves += 1
            log.info(f"  {p['name']} : "
                     + " · ".join(f"{k}={v}" for k, v in lu.items())
                     + f" — « {str(motif)[:70]} »")
        except Exception as exc:
            log.warning(f"  {p.get('name')} : {str(exc)[:70]}")
        log.progress(i / max(1, min(len(cibles), limite)))

    log.info(f"{trouves} rôle(s) suggéré(s), {vides} fiche(s) sans "
             f"mention explicite.")
    if motifs:
        log.info("  raisons : "
                 + " · ".join(f"{k} ({n})"
                              for k, n in sorted(motifs.items(),
                                                 key=lambda x: -x[1])))
    log.info("  Les valeurs suggérées portent une marque sur la fiche. "
             "Les modifier vaut confirmation ; elles ne sont jamais "
