...
FICHIER_PROFILS = Path(__file__).resolve().parent / "tag_profiles.yml"
...
GABARIT_PROFILS = """# tag_profiles.yml — familles de tags et profils de collection.
#
...
def charger(dossier=None) -> dict:
    ...
    table = {
        "familles": {k: list(v)
                     for k, v in PROFILS_DEFAUT["familles"].items()},
        "profils": {k: dict(v) for k, v in PROFILS_DEFAUT["profils"].items()},
        "seuils": dict(PROFILS_DEFAUT["seuils"]),
    }
    chemin = Path(dossier) / "tag_profiles.yml" if dossier else FICHIER_PROFILS
...
def creer_gabarit(dossier=None) -> bool:
    ...
    if chemin.exists():
        return False
    try:
        chemin.write_text(GABARIT_PROFILS, encoding="utf-8")
        return True
    except OSError:
        return False
