...
def vider_cache(ctx):
    """Oublie les réponses de sources mémorisées.

    À lancer si une source a corrigé ses données, ou pour récupérer
    l'espace disque : le cache n'a pas de raison de croître
    indéfiniment sur une collection stable."""
    avant = cache.statistiques()
    if ctx.simulation():
        log.info(f"[simulation] {avant['entrees']} réponse(s) "
                 f"mémorisées, {avant['octets'] // 1024} Kio — "
                 f"rien n'est effacé.")
        return
    n = cache.vider()
    log.info(f"{n} réponse(s) oubliées "
             f"({avant['octets'] // 1024} Kio libérés). "
             f"La prochaine collecte réinterrogera les sources.")
