...
def _wikipedia_concerne(data: Dict, nom: str) -> bool:
    """L'article trouvé parle-t-il bien de la personne cherchée ?

    Wikipédia redirige : demander « X » rend l'article sur la LETTRE
    X, et « Dean » rend une page d'homonymie. Le résumé arrive alors
    dans une biographie sans que rien ne le signale, parce qu'il a
    toutes les apparences d'un vrai texte.

    Le titre rendu doit ressembler au nom demandé — la redirection
    change le titre, c'est ce qui la rend détectable.
    """
    titre = str(data.get("title") or "").strip().lower()
    demande = str(nom or "").strip().lower()
    if not titre or not demande:
        return False
    # L'homonymie se juge AVANT le titre : « Dean » rend une page
    # « Dean » qui ne parle de personne en particulier, et le titre
    # exact ne dit rien là-dessus.
    if data.get("type") == "disambiguation":
        return False
    if titre == demande:
        return True
    # Un titre qui contient le nom demandé reste plausible
    # (« Dean Young (actor) ») ; l'inverse non.
    return demande in titre
...
def fetch_wikipedia_entity(name: str) -> Optional[Dict]:
    ...
    if r:
        try:
            ...
            if (data.get("type") == "standard"
                    and data.get("extract")
                    and _wikipedia_concerne(data, name)):
                out["bio"] = str(data["extract"])[:480]
        except ValueError as exc:
            log.debug(f"réponse Wikipédia illisible : {str(exc)[:70]}")
    ...
    if r:
        try:
            ...
            log.debug(f"source injoignable : {str(exc)[:70]}")
    return out or None
