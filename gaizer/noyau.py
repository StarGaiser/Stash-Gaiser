...
class Context:
    ...
    def ai_for(self, usage: str):
        ...
        if llm.besoin_de_cle(conf) and not cle:
            log.warning(f"aucune clé d'API pour « {fournisseur} » : "
                        f"renseigner le réglage "
                        f"{conf.get('key_setting') or 'llmApiKey'}.")
            return None
    ...
    def simulation(self) -> bool:
        """Mode simulation (dryRun) : rien n'est écrit dans Stash, tout
        est journalisé. Pour éprouver un réglage sans conséquence."""
        return bool(self.settings.get("dryRun", False))
...
def _sans_secrets(valeur):
    ...
    if isinstance(valeur, dict):
        return {k: ("(présente)" if est_secret(k)
