...
FICHIER_FOURNISSEURS = Path(__file__).resolve().parent / "llm_providers.yml"

GABARIT_FOURNISSEURS = """# llm_providers.yml — fournisseurs de modèles de langage.
#
...
def charger(dossier=None) -> dict:
    ...
    table = {k: dict(v) for k, v in FOURNISSEURS_DEFAUT.items()}
    chemin = Path(dossier) / "llm_providers.yml" if dossier else FICHIER_FOURNISSEURS
...
def creer_gabarit(dossier=None) -> bool:
    ...
    if chemin.exists():
        return False
    try:
        chemin.write_text(GABARIT_FOURNISSEURS, encoding="utf-8")
        return True
    except Exception:
        return False
...
def url_pour(conf: dict, reglages: dict) -> str:
    ...
    # Un réglage propre au service s'il subsiste, puis le réglage
    # GÉNÉRIQUE : le service est déjà désigné par le nom du modèle,
    # un champ d'adresse par service laissait quatre cases vides à
    # l'écran pour toujours.
    for nom in (conf.get("url_setting"), "llmUrl"):
        if not nom:
            continue
        valeur = str((reglages or {}).get(nom) or "").strip()
        if valeur:
