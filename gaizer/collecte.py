...
def collecter_stash(ctx: Context, nom: str) -> tuple:
    ...
    if ctx.use_boxes():
        for box in ctx.stash_boxes:
            ...
            hit = cache.lire(cle, "performer", nom, ctx.cache_jours())
            if hit is None:
                try:
                    ...
                    # Un echec n'est PAS memorise comme une reponse :
                    # une panne passagere condamnerait la source pour
                    # toute la duree du cache.
                    continue
                cache.poser(cle, "performer", nom, hit or {})
            if not hit:
                continue
            if hit:
    ...
    for scraper_id in ctx.scrapers():
        ...
        # Une reponse deja obtenue evite un appel de deux a dix
        # secondes. Sur une vingtaine de sources, c'est la difference
        # entre deux minutes par fiche et deux secondes.
        hit = cache.lire(cle, "performer", nom, ctx.cache_jours())
        if hit is None:
            # Une source qui vient d'echouer est ecartee brievement :
            # sans cela, un scraper en panne coute son delai d'attente
            # sur CHAQUE fiche du lot.
            motif = cache.echec_recent(cle, "performer", nom)
            if motif:
                log.debug(f"scraper {cle} ecarte : {motif}")
                continue
            try:
                d = ctx.stash.call_GQL(GQL_SCRAPE_SINGLE, {
                ...
                cache.poser_echec(cle, "performer", nom, str(exc))
                continue
            # « Cette source ne connait pas cette fiche » EST une
            # reponse : ne pas la garder ferait reinterroger
            # indefiniment les sources qui n'ont rien — la majorite.
            cache.poser(cle, "performer", nom, hit or {})
        if not hit:
            continue
        urls.extend(hit.get("urls") or [])
