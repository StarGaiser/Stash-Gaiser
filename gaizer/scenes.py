...
def _coherence_fichier(basename: str, titre: str, studio: str,
                       perfs: list) -> tuple:
    """Le NOM DE FICHIER corrobore-t-il l'identification par EMPREINTE ?
...
def _appliquer_scene(ctx, s, raw, cands, ent, actuel):
    ...
    # URLs des fiches sources : additives, jamais écrasantes
    urls_sc = []
    for d2 in raw.values():
        for u in d2.get("urls") or []:
            if u and u not in (s.get("urls") or []):
                urls_sc.append(u)
    urls_sc = list(dict.fromkeys(urls_sc))
    if urls_sc:
        maj["urls"] = list(dict.fromkeys((s.get("urls") or [])
                                         + urls_sc))
        rapport.append(f"urls: +{len(urls_sc)}")
    # Cover officielle PRIVILÉGIÉE dans tous les cas : l'image du
    # studio est plus représentative qu'une vignette extraite de la
    # vidéo. Elle remplace donc la cover existante (non
    # restaurable) ; applySceneCovers=false pour s'en abstenir.
    cf_cover = None
    if ctx.settings.get("applySceneCovers", True):
        for src2 in sorted(raw, key=lambda x:
            ...
            if img and url_sure(img):
                ...
                rapport.append(f"cover officielle: {src2}")
                break
    if noms_tags:
